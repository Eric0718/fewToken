...
contract FEWToken is ERC20, ERC20Burnable, Ownable {
    ...
    address[] admins;
    uint256 public supply = 2100000000e18;
    uint256 public subscribeFee = 100000000 gwei;
    uint256 public subscribeAmount = 100000e18; //100000e18;
    uint256 public totalSubscribe;
    uint256 public immutable subscribeEndTime;
    uint8 public constant inviteeBase = 3;
    uint8 public rewardsBase = 5;
    ...
    constructor(address controller0,address controller1,address controller2) ERC20("FEWToken", "FEW") {
        ...
        subscribeEndTime = block.timestamp + 50 days;
    ...
    function subscribe(address inviter)public payable{
        ...
        totalSubscribe += subscribeAmount;
    ...
    function claim_E() external{
        ...
        require(amount > 0,"It doesn't meet the conditions");
    ...
    function getInviteInfo(address account) external view returns(bool isInviter,
        uint256 totalInvitationNums,uint256 subscribe_fee,uint256 subscribe_amount) {
        return (invitationInfos[account].canInvite,invitationInfos[account].invitationNums,
        invitationInfos[account].subscribe_E,invitationInfos[account].subscribe_T);
    }

    function setSubscribeFee(uint256 _subFee)public onlyOwner{
        subscribeFee = _subFee;
    }

    function setSubscribeAmount(uint256 _subAmount)public onlyOwner{
        subscribeAmount = _subAmount;
    }

    function setRewardsBase(uint8 _rewBase)public onlyOwner{
        rewardsBase = _rewBase;
    }
    ...
    function withDraw()external onlyOwner{
        require(block.timestamp >= subscribeEndTime,"subscribe not end!");
